...
import dog.aonehax.math.MathEventPublisher;
...
public class GuildChannelStorage {
    ...
    public static TextChannel get(Guild guild) {
        ...
        try {
            return (TextChannel) LiMingShu.getInstance().getCore().getHttpAPI().getChannel(id);
    ...
    public static void loadAll(ConfigurationSection section) {
        ...
        for (Map.Entry<String, Object> entry : values.entrySet()) {
            try {
                ...
                new MathEventPublisher(LiMingShu.getInstance().getCore().getHttpAPI().getGuild(entry.getKey())).start();
