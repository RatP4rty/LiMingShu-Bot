...
import dog.aonehax.events.LevelUpEvent;
...
public class Scoreboard implements ConfigurationSerializable {
    ...
    public Scoreboard(String guildId, Map<String, Integer> map) {
        this(LiMingShu.getInstance().getCore().getHttpAPI().getGuild(guildId), map);
    }
    ...
    private void updateLevel(User user, int score) {
        ...
        if (newLevel > prevLevel) {
            LiMingShu.getInstance().getCore().getEventManager().callEvent(new LevelUpEvent(guild, user, prevLevel, newLevel));
